...
namespace EventBusRabbitMQ
{
       ...
       : IRabbitMQPersistentConnection
    {
        ...
        bool _disposed;
        ILog _logger = log4net.LogManager.GetLogger(typeof(DefaultRabbitMQPersistentConnection));
        ...
        public bool TryConnect()
        {
            ...
            lock (sync_root)
            {
                var policy = RetryPolicy.Handle<SocketException>()
                    ...
                    {
                        _logger.Error(ex+ "RabbitMQ Client could not connect after {TimeOut}s ({ExceptionMessage})"+ $"{time.TotalSeconds:n1}"+ex.Message);
                    }
                );
